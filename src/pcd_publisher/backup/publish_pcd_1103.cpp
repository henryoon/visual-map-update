...
int main(int argc, char** argv)
{
    ...
    if(pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/irol/hj_ws/src/pcd_publisher/pcd_file/304_1103_GFTT10LU001_02.pcd", *original_cloud) == -1)
    {
    ...
    *filtered_cloud = *original_cloud;  // Copy the original point cloud to the filtered point cloud
    ...
    float selected_point_x = 0.18;  // Example selected point x-coordinate
    float selected_point_y = 1.5;  // Example selected point y-coordinate
    float selected_point_z = 0.85;  // Example selected point z-coordinate

    float x_range = 0.35;  // The range around the x-coordinate to filter out
    float y_range = 2.0;  // The range around the y-coordinate to filter out
    ...
    *filtered_cloud = *x_filtered + *y_filtered + *z_filtered;
    ...
    original_output.header.frame_id = "base_link";

    pcl::toROSMsg(*filtered_cloud, filtered_output);
    filtered_output.header.frame_id = "base_link";
