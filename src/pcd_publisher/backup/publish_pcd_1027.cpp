...
int main(int argc, char** argv)
{
    ...
    if(pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/irol/hj_ws/src/pcd_publisher/pcd_file/304_afterimage.pcd", *original_cloud) == -1)
    {
    ...
    *filtered_cloud = *original_cloud;  // Copy the original point cloud to the filtered point cloud
    ...
    float selected_point_y = 0.5;  // Example selected point y-coordinate
    float selected_point_z = 0.0;  // Example selected point z-coordinate

    float x_range = 1.0;  // The range around the x-coordinate to filter out
    float y_range = 1.0;  // The range around the y-coordinate to filter out
    float z_range = 1.0;  // The range around the z-coordinate to filter out
