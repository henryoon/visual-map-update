...
def publish_base_to_point(publisher):
    ...
    for tag_id, point in c2p_tf.items():
        ...
        base_to_targetPoint = [
            int(tag_id),
            round(b2t_tf[0] + point[0], 3) + 0.01,
            round(b2t_tf[1] + point[1], 3) + 0.03,
            round(b2t_tf[2] + point[2], 3) + 0.135 
        ]
        ...
        # rospy.loginfo(f"ID: {base_to_targetPoint[0]}, B2P_position {base_to_targetPoint[1], base_to_targetPoint[2], base_to_targetPoint[3]}")       
